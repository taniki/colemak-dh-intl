; Generated by kalamine on 2024-04-16

#NoEnv
#Persistent
#InstallKeybdHook
#SingleInstance,       force
#MaxThreadsBuffer
#MaxThreadsPerHotKey   3
#MaxHotkeysPerInterval 300
#MaxThreads            20

SendMode Event ; either Event or Input
SetKeyDelay,   -1
SetBatchLines, -1
Process, Priority, , R
SetWorkingDir, %A_ScriptDir%
StringCaseSense, On


;-------------------------------------------------------------------------------
; On/Off Switch
;-------------------------------------------------------------------------------

global Active := True

HideTrayTip() {
  TrayTip  ; Attempt to hide it the normal way.
  if SubStr(A_OSVersion,1,3) = "10." {
    Menu Tray, NoIcon
    Sleep 200  ; It may be necessary to adjust this sleep.
    Menu Tray, Icon
  }
}

ShowTrayTip() {
  title := "colemakdh-tk"
  text := Active ? "ON" : "OFF"
  HideTrayTip()
  TrayTip, %title% , %text%, 1, 0x31
  SetTimer, HideTrayTip, -1500
}

RAlt & Alt::
Alt & RAlt::
  global Active
  Active := !Active
  ShowTrayTip()
  return

#If Active
SetTimer, ShowTrayTip, -1000  ; not working


;-------------------------------------------------------------------------------
; DeadKey Helpers
;-------------------------------------------------------------------------------

global DeadKey := ""

; Check CapsLock status, upper the char if needed and send the char
SendChar(char) {
  if % GetKeyState("CapsLock", "T") {
    if (StrLen(char) == 6) {
      ; we have something in the form of `U+NNNN `
      ; Change it to 0xNNNN so it can be passed to `Chr` function
      char := Chr("0x" SubStr(char, 3, 4))
    }
    StringUpper, char, char
  }
  Send, {%char%}
}

DoTerm(base:="") {
  global DeadKey

  term := SubStr(DeadKey, 2, 1)

  Send, {%term%}
  SendChar(base)
  DeadKey := ""
}

DoAction(action:="") {
  global DeadKey

  if (action == "U+0020") {
    Send, {SC39}
    DeadKey := ""
  }
  else if (StrLen(action) != 2) {
    SendChar(action)
    DeadKey := ""
  }
  else if (action == DeadKey) {
    DoTerm(SubStr(DeadKey, 2, 1))
  }
  else {
    DeadKey := action
  }
}

SendKey(base, deadkeymap) {
  if (!DeadKey) {
    DoAction(base)
  }
  else if (deadkeymap.HasKey(DeadKey)) {
    DoAction(deadkeymap[DeadKey])
  }
  else {
    DoTerm(base)
  }
}


;-------------------------------------------------------------------------------
; Base
;-------------------------------------------------------------------------------

;  Digits

 SC02::SendKey("U+0031", {"*^": "U+00b9"}) ; 1
+SC02::SendKey("U+0021", {}) ; !

 SC03::SendKey("U+0032", {"**": "U+00ab", "*^": "U+00b2"}) ; 2
+SC03::SendKey("U+0040", {}) ; @

 SC04::SendKey("U+0033", {"**": "U+00bb", "*^": "U+00b3"}) ; 3
+SC04::SendKey("U+0023", {}) ; #

 SC05::SendKey("U+0034", {"*^": "U+2074"}) ; 4
+SC05::SendKey("U+0024", {}) ; $

 SC06::SendKey("U+0035", {"**": "U+20ac", "*^": "U+2075"}) ; 5
+SC06::SendKey("U+0025", {}) ; %

 SC07::SendKey("U+0036", {"*^": "U+2076"}) ; 6
+SC07::SendKey("U+005e", {}) ; ^

 SC08::SendKey("U+0037", {"*^": "U+2077"}) ; 7
+SC08::SendKey("U+0026", {}) ; &

 SC09::SendKey("U+0038", {"*^": "U+2078"}) ; 8
+SC09::SendKey("U+002a", {}) ; *

 SC0a::SendKey("U+0039", {"*^": "U+2079"}) ; 9
+SC0a::SendKey("U+0028", {"*^": "U+207d"}) ; (

 SC0b::SendKey("U+0030", {"*^": "U+2070"}) ; 0
+SC0b::SendKey("U+0029", {"*^": "U+207e"}) ; )

;  Letters, first row

 SC10::SendKey("U+0071", {}) ; q
+SC10::SendKey("U+0051", {}) ; Q

 SC11::SendKey("U+0077", {"*``": "U+1e81", "*´": "U+1e83", "*^": "U+0175", "*¨": "U+1e85"}) ; w
+SC11::SendKey("U+0057", {"*``": "U+1e80", "*´": "U+1e82", "*^": "U+0174", "*¨": "U+1e84"}) ; W

 SC12::SendKey("U+0066", {"**": "U+00e9"}) ; f
+SC12::SendKey("U+0046", {"**": "U+00c9"}) ; F

 SC13::SendKey("U+0070", {"*´": "U+1e55"}) ; p
+SC13::SendKey("U+0050", {"*´": "U+1e54"}) ; P

 SC14::SendKey("U+0062", {}) ; b
+SC14::SendKey("U+0042", {}) ; B

 SC15::SendKey("U+006a", {"**": "U+00fd", "*^": "U+0135"}) ; j
+SC15::SendKey("U+004a", {"**": "U+00dd", "*^": "U+0134"}) ; J

 SC16::SendKey("U+006c", {"**": "U+00fa", "*´": "U+013a"}) ; l
+SC16::SendKey("U+004c", {"**": "U+00da", "*´": "U+0139"}) ; L

 SC17::SendKey("U+0075", {"**": "U+00ed", "*``": "U+00f9", "*´": "U+00fa", "*^": "U+00fb", "*~": "U+0169", "*¨": "U+00fc"}) ; u
+SC17::SendKey("U+0055", {"**": "U+00cd", "*``": "U+00d9", "*´": "U+00da", "*^": "U+00db", "*~": "U+0168", "*¨": "U+00dc"}) ; U

 SC18::SendKey("U+0079", {"**": "U+00f3", "*``": "U+1ef3", "*´": "U+00fd", "*^": "U+0177", "*~": "U+1ef9", "*¨": "U+00ff"}) ; y
+SC18::SendKey("U+0059", {"**": "U+00d3", "*``": "U+1ef2", "*´": "U+00dd", "*^": "U+0176", "*~": "U+1ef8", "*¨": "U+0178"}) ; Y

 SC19::SendKey("U+003b", {}) ; ;
+SC19::SendKey("U+003a", {}) ; :

;  Letters, second row

 SC1e::SendKey("U+0061", {"**": "U+00e1", "*``": "U+00e0", "*´": "U+00e1", "*^": "U+00e2", "*~": "U+00e3", "*¨": "U+00e4"}) ; a
+SC1e::SendKey("U+0041", {"**": "U+00c1", "*``": "U+00c0", "*´": "U+00c1", "*^": "U+00c2", "*~": "U+00c3", "*¨": "U+00c4"}) ; A

 SC1f::SendKey("U+0072", {"*´": "U+0155"}) ; r
+SC1f::SendKey("U+0052", {"*´": "U+0154"}) ; R

 SC20::SendKey("U+0073", {"*´": "U+015b", "*^": "U+015d"}) ; s
+SC20::SendKey("U+0053", {"*´": "U+015a", "*^": "U+015c"}) ; S

 SC21::SendKey("U+0074", {"*¨": "U+1e97"}) ; t
+SC21::SendKey("U+0054", {}) ; T

 SC22::SendKey("U+0067", {"*´": "U+01f5", "*^": "U+011d"}) ; g
+SC22::SendKey("U+0047", {"*´": "U+01f4", "*^": "U+011c"}) ; G

 SC23::SendKey("U+006d", {"*´": "U+1e3f"}) ; m
+SC23::SendKey("U+004d", {"*´": "U+1e3e"}) ; M

 SC24::SendKey("U+006e", {"*``": "U+01f9", "*´": "U+0144", "*~": "U+00f1"}) ; n
+SC24::SendKey("U+004e", {"*``": "U+01f8", "*´": "U+0143", "*~": "U+00d1"}) ; N

 SC25::SendKey("U+0065", {"*``": "U+00e8", "*´": "U+00e9", "*^": "U+00ea", "*~": "U+1ebd", "*¨": "U+00eb"}) ; e
+SC25::SendKey("U+0045", {"*``": "U+00c8", "*´": "U+00c9", "*^": "U+00ca", "*~": "U+1ebc", "*¨": "U+00cb"}) ; E

 SC26::SendKey("U+0069", {"*``": "U+00ec", "*´": "U+00ed", "*^": "U+00ee", "*~": "U+0129", "*¨": "U+00ef"}) ; i
+SC26::SendKey("U+0049", {"*``": "U+00cc", "*´": "U+00cd", "*^": "U+00ce", "*~": "U+0128", "*¨": "U+00cf"}) ; I

 SC27::SendKey("U+006f", {"*``": "U+00f2", "*´": "U+00f3", "*^": "U+00f4", "*~": "U+00f5", "*¨": "U+00f6"}) ; o
+SC27::SendKey("U+004f", {"*``": "U+00d2", "*´": "U+00d3", "*^": "U+00d4", "*~": "U+00d5", "*¨": "U+00d6"}) ; O

;  Letters, third row

 SC2c::SendKey("U+007a", {"*´": "U+017a", "*^": "U+1e91"}) ; z
+SC2c::SendKey("U+005a", {"*´": "U+0179", "*^": "U+1e90"}) ; Z

 SC2d::SendKey("U+0078", {"*¨": "U+1e8d"}) ; x
+SC2d::SendKey("U+0058", {"*¨": "U+1e8c"}) ; X

 SC2e::SendKey("U+0063", {"**": "U+00e7", "*´": "U+0107", "*^": "U+0109"}) ; c
+SC2e::SendKey("U+0043", {"**": "U+00c7", "*´": "U+0106", "*^": "U+0108"}) ; C

 SC2f::SendKey("U+0064", {}) ; d
+SC2f::SendKey("U+0044", {}) ; D

 SC30::SendKey("U+0076", {"*~": "U+1e7d"}) ; v
+SC30::SendKey("U+0056", {"*~": "U+1e7c"}) ; V

 SC31::SendKey("U+006b", {"*´": "U+1e31"}) ; k
+SC31::SendKey("U+004b", {"*´": "U+1e30"}) ; K

 SC32::SendKey("U+0068", {"**": "U+00b5", "*^": "U+0125", "*¨": "U+1e27"}) ; h
+SC32::SendKey("U+0048", {"*^": "U+0124", "*¨": "U+1e26"}) ; H

 SC33::SendKey("U+002c", {"**": "U+00b7"}) ; ,
+SC33::SendKey("U+003c", {"**": "U+2022", "*~": "U+2272"}) ; <

 SC34::SendKey("U+002e", {"**": "U+2026"}) ; .
+SC34::SendKey("U+003e", {"*~": "U+2273"}) ; >

 SC35::SendKey("U+002f", {}) ; /
+SC35::SendKey("U+003f", {}) ; ?

;  Pinky keys

 SC0c::SendKey("U+002d", {"*^": "U+207b"}) ; -
+SC0c::SendKey("U+005f", {}) ; _

 SC0d::SendKey("U+003d", {"*^": "U+207c", "*~": "U+2243"}) ; =
+SC0d::SendKey("U+002b", {"*^": "U+207a"}) ; +

 SC1a::SendKey("U+005b", {}) ; [
+SC1a::SendKey("U+007b", {}) ; {

 SC1b::SendKey("U+005d", {}) ; ]
+SC1b::SendKey("U+007d", {}) ; }

 SC28::SendKey("**", {"**": "'"})
+SC28::SendKey("*¨", {"*¨": "¨"})

 SC29::SendKey("U+0060", {}) ; `
+SC29::SendKey("U+007e", {}) ; ~

 SC2b::SendKey("U+005c", {}) ; \
+SC2b::SendKey("U+007c", {}) ; |

;  Space bar

 SC39::SendKey("U+0020", {"**": "U+0027", "*``": "U+0060", "*´": "U+0027", "*^": "U+005e", "*~": "U+007e", "*¨": "U+0022"}) ;  
+SC39::SendKey("U+0020", {"**": "U+0027", "*``": "U+0060", "*´": "U+0027", "*^": "U+005e", "*~": "U+007e", "*¨": "U+0022"}) ;  


;-------------------------------------------------------------------------------
; AltGr
;-------------------------------------------------------------------------------

;  Digits

 <^>!SC07::SendKey("*^", {"*^": "^"})

;  Letters, first row

 <^>!SC10::SendKey("U+0040", {}) ; @

 <^>!SC11::SendKey("U+003c", {"**": "U+2022", "*~": "U+2272"}) ; <
<^>!+SC11::SendKey("U+2264", {}) ; ≤

 <^>!SC12::SendKey("U+003e", {"*~": "U+2273"}) ; >
<^>!+SC12::SendKey("U+2265", {}) ; ≥

 <^>!SC13::SendKey("U+0024", {}) ; $

 <^>!SC14::SendKey("U+0025", {}) ; %

 <^>!SC15::SendKey("U+005e", {}) ; ^

 <^>!SC16::SendKey("U+0026", {}) ; &

 <^>!SC17::SendKey("U+002a", {}) ; *

 <^>!SC18::SendKey("U+0027", {}) ; '

 <^>!SC19::SendKey("U+0060", {}) ; `

;  Letters, second row

 <^>!SC1e::SendKey("U+007b", {}) ; {

 <^>!SC1f::SendKey("U+0028", {"*^": "U+207d"}) ; (

 <^>!SC20::SendKey("U+0029", {"*^": "U+207e"}) ; )

 <^>!SC21::SendKey("U+007d", {}) ; }

 <^>!SC22::SendKey("U+003d", {"*^": "U+207c", "*~": "U+2243"}) ; =

 <^>!SC23::SendKey("U+005c", {}) ; \

 <^>!SC24::SendKey("U+002b", {"*^": "U+207a"}) ; +

 <^>!SC25::SendKey("U+002d", {"*^": "U+207b"}) ; -

 <^>!SC26::SendKey("U+002f", {}) ; /

 <^>!SC27::SendKey("U+0022", {}) ; "

;  Letters, third row

 <^>!SC2c::SendKey("U+007e", {}) ; ~

 <^>!SC2d::SendKey("U+005b", {}) ; [

 <^>!SC2e::SendKey("U+005d", {}) ; ]

 <^>!SC2f::SendKey("U+005f", {}) ; _

 <^>!SC30::SendKey("U+0023", {}) ; #

 <^>!SC31::SendKey("U+007c", {}) ; |
<^>!+SC31::SendKey("U+00a6", {}) ; ¦

 <^>!SC32::SendKey("U+0021", {}) ; !

 <^>!SC33::SendKey("U+003b", {}) ; ;

 <^>!SC34::SendKey("U+003a", {}) ; :

 <^>!SC35::SendKey("U+003f", {}) ; ?

;  Pinky keys

 <^>!SC28::SendKey("*´", {"*´": "´"})
<^>!+SC28::SendKey("*¨", {"*¨": "¨"})

 <^>!SC29::SendKey("*``", {"*``": "`"}) ; *`
<^>!+SC29::SendKey("*~", {"*~": "~"})

;  Space bar

 <^>!SC39::SendKey("U+0020", {"**": "U+0027", "*``": "U+0060", "*´": "U+0027", "*^": "U+005e", "*~": "U+007e", "*¨": "U+0022"}) ;  
<^>!+SC39::SendKey("U+0020", {"**": "U+0027", "*``": "U+0060", "*´": "U+0027", "*^": "U+005e", "*~": "U+007e", "*¨": "U+0022"}) ;  

; Special Keys

$<^>!Esc::       Send {SC01}
$<^>!End::       Send {SC4f}
$<^>!Home::      Send {SC47}
$<^>!Delete::    Send {SC53}
$<^>!Backspace:: Send {SC0e}


;-------------------------------------------------------------------------------
; Ctrl
;-------------------------------------------------------------------------------

;  Digits

;  Letters, first row

 ^SC10::Send  ^q
^+SC10::Send ^+Q

 ^SC11::Send  ^w
^+SC11::Send ^+W

 ^SC12::Send  ^f
^+SC12::Send ^+F

 ^SC13::Send  ^p
^+SC13::Send ^+P

 ^SC14::Send  ^b
^+SC14::Send ^+B

 ^SC15::Send  ^j
^+SC15::Send ^+J

 ^SC16::Send  ^l
^+SC16::Send ^+L

 ^SC17::Send  ^u
^+SC17::Send ^+U

 ^SC18::Send  ^y
^+SC18::Send ^+Y

;  Letters, second row

 ^SC1e::Send  ^a
^+SC1e::Send ^+A

 ^SC1f::Send  ^r
^+SC1f::Send ^+R

 ^SC20::Send  ^s
^+SC20::Send ^+S

 ^SC21::Send  ^t
^+SC21::Send ^+T

 ^SC22::Send  ^g
^+SC22::Send ^+G

 ^SC23::Send  ^m
^+SC23::Send ^+M

 ^SC24::Send  ^n
^+SC24::Send ^+N

 ^SC25::Send  ^e
^+SC25::Send ^+E

 ^SC26::Send  ^i
^+SC26::Send ^+I

 ^SC27::Send  ^o
^+SC27::Send ^+O

;  Letters, third row

 ^SC2c::Send  ^z
^+SC2c::Send ^+Z

 ^SC2d::Send  ^x
^+SC2d::Send ^+X

 ^SC2e::Send  ^c
^+SC2e::Send ^+C

 ^SC2f::Send  ^d
^+SC2f::Send ^+D

 ^SC30::Send  ^v
^+SC30::Send ^+V

 ^SC31::Send  ^k
^+SC31::Send ^+K

 ^SC32::Send  ^h
^+SC32::Send ^+H

;  Pinky keys

;  Space bar

